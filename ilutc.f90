...
!|--------------------------------------------------------------------*/
   ...
   int rowz, *rowj=NULL, imax, icut, *iperm=ilusch->perm2;
   double *rowm=NULL, xmax, xmax0, tmp;
   int dec, decnt=0;
...
!#endif
      ...
      for (k=0; k<rowz; k++)
	 tnorm += fabs(rowm[k]);
      if(fabs(tnorm-DBL_EPSILON) <= DBL_EPSILON*fabs(tnorm)) goto label9991;
...
!*/
	 ...
	 ilusch->L->pa[ii] = (double *) Malloc(len*sizeof(double), "ilutpC:6"); 
	 memcpy(ilusch->L->pa[ii], w, len*sizeof(double));
	 for (j=0; j<len; j++)
...
!#endif
	 ...
	 if ( fabs(w[ii+j]) > drop6*tnorm ) {
      ...
      ilusch->U->pa[ii] = (double *) Malloc(len*sizeof(double), "ilutpC:7" ); 
...
! */
      if(fabs(w[ii]-DBL_EPSILON) <= DBL_EPSILON*fabs(w[ii])) w[ii] = (0.0001+drop6)*tnorm;
      ilusch->U->pa[ii][0] = 1.0 / w[ii];
      ilusch->U->pj[ii][0] = ii;
      memcpy(&ilusch->U->pa[ii][1], &w[ii+1], (len-1)*sizeof(double));
      lenu = 0;
      for (k=ii+1; k<ii+len; k++)
...
!|--------------------------------------------------------------------*/
   ...
   for (ii=0; ii<rmax; ii++) {
      ...
      rowz = amat->nnzrow[ii];
      for (k=0; k<rowz; k++)
         if(fabs(rowm[k]-DBL_EPSILON) > DBL_EPSILON*fabs(rowm[k])) goto label40;
      goto label9996;
!/*---------------------------------------------------------------------
!|     unpack amat in arrays w, jw, jwrev
!|     WE ASSUME THERE IS A DIAGONAL ELEMENT
...
!|--------------------------------------------------------------------*/
	 ...
	 ilusch->L->pa[ii] = (double *) Malloc(lenl*sizeof(double), "ilutD:5"); 
	 memcpy(ilusch->L->pj[ii], jw, lenl*sizeof(int));
	 memcpy(ilusch->L->pa[ii], w, lenl*sizeof(double));
      }
!/*---------------------------------------------------------------------
...
!|--------------------------------------------------------------------*/
	 ...
	 if ( fabs(w[ii+j]) > drop6*tnorm ) {
      ...
      ilusch->U->pa[ii] = (double *) Malloc(lenu*sizeof(double), "ilutD:6" ); 
      ilusch->U->pj[ii] = (int *) Malloc(lenu*sizeof(int), "ilutD:7" ); 
      if(fabs(t-DBL_EPSILON) <= DBL_EPSILON*fabs(t)) t=(0.0001+drop6)*tnorm;
...
!|--------------------------------------------------------------------*/
      ...
      memcpy(&ilusch->U->pa[ii][1], w, jpos*sizeof(double));
   }
!/*---------------------------------------------------------------------
!|     end main loop - now do clean up
!|--------------------------------------------------------------------*/
