...
/*--------------------  allocate space  */
    for (i=0; i<size; i++) {
      ...
      bmat->nnzrow[i] = ind[i];
      if (job == 1) {
	bmat->pa[i] = (double *) Malloc(ind[i]*sizeof(double), "SparTran:3" );
      }
      ind[i] = 0;
    }
  }
...
/*   local variables    */
   ...
   for (i=0; i<mata->n; i++) {
	 ...
	 for (k=0; k<mata->nnzrow[i]; k++)
      ...
      if(fabs(scal-DBL_EPSILON) <= DBL_EPSILON*fabs(scal)){
	scal = 1.0; 
	/* YS. return i+1; */
      }
      else 
...
|--------------------------------------*/
   for (i=0; i<mata->n; i++) {
      if(fabs(diag[i]-DBL_EPSILON) <= DBL_EPSILON*fabs(diag[i]))
	/* return i+1;*/
	diag[i] = 1.0; 
      else 
	 diag[i] = 1.0 / diag[i];
...
/*
  void qsortC(int *, double *, int, int, int);
  void qsort2C(int *, double *, int, int, int);
*/  
  if (hilo)
    for (j=0; j<n; j++)
