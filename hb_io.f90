...
!
!    Local, integer ( kind = 4 ) INDCRD, the number of input lines for row indices.
!
...
!
!    Local, integer ( kind = 4 ) NNZERO.  In the case of assembled sparse matrices,
...
!
!    Local, integer ( kind = 4 ) NELTVL, the number of finite element matrix entries,
...
!
!    Local, integer ( kind = 4 ) COLPTR(NCOL+1), COLPTR(I) points to the location of
...
!
  ...
  integer   ( kind = 4 ), target, save, allocatable, dimension ( : )    :: colptr
  real      ( kind = 8 ), save, allocatable, dimension ( :, : ) :: exact
  real      ( kind = 8 ), save, allocatable, dimension ( :, : ) :: guess
  integer   ( kind = 4 ), save, allocatable, dimension ( : )    :: rhsind
  integer   ( kind = 4 ), save, allocatable, dimension ( : )    :: rhsptr
  real      ( kind = 8 ), save, allocatable, dimension ( :, : ) :: rhsval
  real      ( kind = 8 ), save, allocatable, dimension ( : )    :: rhsvec
  integer   ( kind = 4 ), target, save, allocatable, dimension ( : )    :: rowind
  real      ( kind = 8 ), target, save, allocatable, dimension ( : )    :: values
...
subroutine hb_exact_read ( input_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, exact )
!*****************************************************************************80
!
...
!
  ...
  return
end subroutine hb_exact_read
!*****************************************************************************



subroutine hb_exact_write ( output_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, exact )
!*****************************************************************************80
!
...
!
  if ( 0 < ptrcrd ) then
    if ( allocated ( colptr ) ) then
      deallocate ( colptr,STAT=istat )
    end if
    allocate ( colptr(ncol+1),STAT=istat )
  end if

  if ( 0 < indcrd ) then
    if ( allocated ( rowind ) ) then
      deallocate ( rowind,STAT=istat )
    end if

    if ( mxtype(3:3) == 'A' ) then
      allocate ( rowind(nnzero),STAT=istat )
    else if ( mxtype(3:3) == 'E' ) then
      allocate ( rowind(neltvl),STAT=istat )
    else
      write ( *, '(a)' ) ' '
...
!
  if ( 0 < valcrd ) then
    ...
    if ( mxtype(3:3) == 'A' ) then
      allocate ( values(1:nnzero),STAT=istat )
    else if ( mxtype(3:3) == 'E' ) then
      allocate ( values(1:neltvl),STAT=istat )
    else
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'HB_FILE_READ - Fatal error!'
      write ( *, '(a)' ) '  Illegal value of MXTYPE(3:3)!'
    ...
    call hb_values_read(input_unit, valcrd, mxtype, nnzero, neltvl, valfmt, values)
  end if
!
  ...
  if ( 0 < rhscrd ) then
    if ( rhstyp(1:1) == 'F' ) then
      if ( allocated ( rhsval ) ) then
        deallocate ( rhsval,STAT=istat )
      end if
      allocate ( rhsval(nrow,nrhs) )
    else if ( rhstyp(1:1) == 'M' .and. mxtype(3:3) == 'A' ) then
      if ( allocated ( rhsptr ) ) then
        deallocate ( rhsptr,STAT=istat )
      end if
      allocate ( rhsptr(nrhs+1),STAT=istat )
      if ( allocated ( rhsind ) ) then
        deallocate ( rhsind,STAT=istat )
      end if
      allocate ( rhsind(nrhsix),STAT=istat )
      if ( allocated ( rhsvec ) ) then
        deallocate ( rhsvec ,STAT=istat)
      end if
      allocate ( rhsvec(nrhsix),STAT=istat )
...
!
    if ( rhstyp(2:2) == 'G' ) then
      if ( allocated ( guess ) ) then
        deallocate ( guess,STAT=istat )
      end if
      allocate ( guess(1:nrow,1:nrhs),STAT=istat )
      call hb_guess_read(input_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, guess)
    end if
!
...
!
    if ( rhstyp(3:3) == 'X' ) then
      if ( allocated ( exact ) ) then
        deallocate ( exact,STAT=istat )
      end if
      allocate ( exact(1:nrow,1:nrhs),STAT=istat )
      call hb_exact_read (input_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, exact)
    end if
  end if

  return
end subroutine hb_file_read
!*****************************************************************************
...
!
!    Input, integer ( kind = 4 ) INDCRD, the number of input lines for row indices.
!
...
!    number of entries in a single unassembled right hand side vector.
!
!    Input, integer ( kind = 4 ) NELTVL, the number of finite element matrix entries,
!    set to 0 in the case of assembled matrices.
!
...
!    in the case of unassembled matrices.)  Ignored if NRHS = 0.
!
!    Input, integer ( kind = 4 ) COLPTR(NCOL+1), COLPTR(I) points to the location of
!    the first entry of column I in the sparse matrix structure.
!
!    If MXTYPE(3:3) == 'A':
!
...
!
  call hb_guess_write ( output_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, guess )
!
!  Write the exact solutions.
!
  call hb_exact_write ( output_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, exact )

  return
end subroutine hb_file_write
!*****************************************************************************
...
subroutine hb_guess_read ( input_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, guess )
...
subroutine hb_guess_write ( output_unit, nrow, nrhs, rhscrd, rhsfmt, rhstyp, guess )
!*****************************************************************************80
!
...
!    Input, integer ( kind = 4 ) NRHS, the number of right hand sides.
!
!    Input, integer ( kind = 4 ) NRHSIX, the number of entries of storage for right
...
!
  ...
  return
end subroutine hb_header_print
!*****************************************************************************
...
!  Read the header block.
  ...
  !read number of input file rows for col(ia),row(ja),value,rhs 
  ...
  !read matrix_type, num rows, num cols, num non zero entries, number of elements
  ...
  !read formats for pointer,index,value,and rhs
  ...
  !read right hand side data if exists
  if ( 0 < rhscrd ) then
...
!
  ...
  return
end subroutine hb_header_write
!*****************************************************************************
...
subroutine hb_matvec_a_mem ( nrow, ncol, nnzero, nrhs, colptr, rowind, values, exact, rhsval )
!*****************************************************************************80
!
...
!    B(I,1:NRHS) = B(I,1:NRHS) + A(I,J) * X(J,1:NRHS)
  ...
  return
end subroutine hb_matvec_a_mem
!*****************************************************************************
...
!
  ...
  return
end subroutine hb_structure_print
!*****************************************************************************
...
!
  ...
  return
end subroutine hb_structure_read
!*****************************************************************************
...
!
  ...
  return
end subroutine hb_structure_write
!*****************************************************************************
...
!
  ...
  return
end subroutine hb_values_print
!*****************************************************************************




subroutine hb_values_read ( input_unit, valcrd, mxtype, nnzero, neltvl, valfmt, values )
!*****************************************************************************80
!
...
!    case "E" = unassembled finite element matrices.
  ...
  return
end subroutine hb_values_read
!*****************************************************************************




subroutine hb_values_write ( output_unit, valcrd, mxtype, nnzero, neltvl, valfmt, values )
!*****************************************************************************80
!
...
!
  ...
  return
end subroutine hb_values_write
!*****************************************************************************




subroutine hb_vecmat_a_mem ( nrow, ncol, nnzero, nrhs, colptr, rowind, values, exact, rhsval )
!*****************************************************************************80
!
...
!    B(J,1:NRHS) = B(J,1:NRHS) + X(I,1:NRHS) * A(I,J)
  ...
  return
end subroutine hb_vecmat_a_mem
!*****************************************************************************
